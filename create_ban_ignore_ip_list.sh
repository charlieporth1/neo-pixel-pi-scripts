...
ulimit -n $(( 8192 * 2))
ulimit -n
source $PROG/all-scripts-exports-lite.sh --no-log || source $PROG/all-scripts-exports.sh --no-log
CONCURRENT
alias ip-subnet-ignore='grep -vE "^(:/)$"'
dig www.robtex.com uptimerobot.com www.ipqualityscore.com > /dev/null
dns_ip_subnet=$(bash $PROG/get_ext_ip.sh dns.ctptech.dev | ip-sort-subnet)
dns_ip_subnet_v6=$(dig -t aaaa dns.ctptech.dev | ip-sort-subnet)

network_interface_ip_subnets=$(bash $PROG/get_network_devices_ip_address.sh --all | ip-sort-subnet)
current_ip_address_v4=$(bash $PROG/get_ext_ip.sh --multiple-nic | ip-sort-subnet)
current_ip_address_v6=$(bash $PROG/get_ext_ip.sh -6 --multiple-nic | ip-sort-subnet)
current_ip_address=$(bash $PROG/get_ext_ip.sh --current-ip | ip-sort-subnet)

dns_ip=$(bash $PROG/get_ext_ip.sh dns.ctptech.dev | ip-sort-subnet)
home_ip=$(bash $PROG/get_ext_ip.sh home.ctptech.dev | ip-sort-subnet)
gcp_ip=$(bash $PROG/get_ext_ip.sh gcp.ctptech.dev | ip-sort-subnet)
...
IPv4_ROUTE=$(sudo ip route list | ip-sort-subnet)
IPv6_ROUTE=$(sudo ip -6 route list | ip-sort-subnet)

IP_ADDR=$(sudo ip addr | ip-sort-subnet)
IPv4_ADDR=$(sudo ip -4 addr | ip-sort-subnet)
IPv6_ADDR=$(sudo ip -6 addr | ip-sort-subnet)

tailscale_ip_addresses=$(tailscale status | awk '{print $1}' | ip-sort-subnet)
current_tailscale_ip_addresses=$(tailscale ip | ip-sort-subnet)
...
function from_asn_to_ip_subnet_range() {
	local asn="$1"
 	# https://superuser.com/questions/405666/how-to-find-out-all-ip-ranges-belonging-to-a-certain-as
	# IPv4
	whois -h whois.radb.net -- "-i origin $asn" | grepip -4 | grep -Eo "$IP_REGEX_PORT_SUBNET"
	# IPv6
#	whois -h whois.radb.net -- "!6as714' -i origin $asn" | grep -Eo "$IPV6_FULL_REGEX_SUBNET"
	whois -h whois.radb.net -- "-i origin $asn" | grepip -6 | grep -Eo "$IPV6_REGEX_SUBNET"
	whois -h whois.radb.net -- "\!6$asn" | grepip -6 | grep -Eo "$IPV6_REGEX_SUBNET"
}
export -f from_asn_to_ip_subnet_range
...
UNI_IGNORE_IP=(
	...
	$current_ip_address_v4
	$current_ip_address_v6
	$current_ip_address
	$dns_ip
	$home_ip
	$gcp_ip
	...
	$( parallel echo '{}/32' ::: $dns_ip_subnet )
	$( parallel echo '{}/24' ::: $dns_ip_subnet )
	$( parallel echo '{}/16' ::: $dns_ip_subnet )
	$( parallel echo '{}/8' ::: $dns_ip_subnet )

	$( parallel echo '{}/32' ::: $tailscale_ip_addresses )
	$( parallel echo '{}/24' ::: $tailscale_ip_addresses )
	$( parallel echo '{}/16' ::: $tailscale_ip_addresses )
	$( parallel echo '{}/8' ::: $tailscale_ip_addresses )

	$( parallel echo '{}/32' ::: $current_tailscale_ip_addresses )
	$( parallel echo '{}/24' ::: $current_tailscale_ip_addresses )
	$( parallel echo '{}/16' ::: $current_tailscale_ip_addresses )
	$( parallel echo '{}/8' ::: $current_tailscale_ip_addresses )

	$( parallel echo '{}/32' ::: $network_interface_ip_subnets )
	$( parallel echo '{}/24' ::: $network_interface_ip_subnets )
	$( parallel echo '{}/16' ::: $network_interface_ip_subnets )
	$( parallel echo '{}/8' ::: $network_interface_ip_subnets )


	$( parallel echo '{}/32' ::: $IPv4_ROUTE )
	$( parallel echo '{}/24' ::: $IPv4_ROUTE )
	$( parallel echo '{}/16' ::: $IPv4_ROUTE )
	$( parallel echo '{}/8' ::: $IPv4_ROUTE )

	$( parallel echo '{}/32' ::: $current_ip_address )
	$( parallel echo '{}/24' ::: $current_ip_address )
	$( parallel echo '{}/16' ::: $current_ip_address )
	$( parallel echo '{}/8' ::: $current_ip_address )

	$( parallel echo '{}/32' ::: $IPv4_ADDR )
	$( parallel echo '{}/24' ::: $IPv4_ADDR )
	$( parallel echo '{}/16' ::: $IPv4_ADDR )
	$( parallel echo '{}/8' ::: $IPv4_ADDR )
	...
	$(
		if [ "$IS_SOURCED" == 'false' ]; then
			...
			do
				# https://superuser.com/questions/405666/how-to-find-out-all-ip-ranges-belonging-to-a-certain-as
				# IPv4
				from_asn_to_ip_subnet_range "$asn"
			done
			for asn in "${T_MobileIPs_SPRINT_asn[@]}"
			do
				# IPv4
...
IPv4_subnets_list=$(seq 0 32 | xargs)
IPv6_subnets_list=$(seq 0 128 | xargs)
IPv4_subnets_list=${IPv4_subnets_list//\ /\ /}
IPv6_subnets_list=${IPv6_subnets_list//\ /\ /}
...
subnet_ignore_list=(
	...
	127.0.0.0/8
	$(
		parallel $parallel_args printf '%-1s%-1s\\n' "{}" ::: $current_ip_address $current_ip_address_v4 ::: $IPv4_subnets_list
		parallel $parallel_args printf '%-1s%-1s\\n' "{}" ::: $current_ip_address_v6 ::: $IPv6_subnets_list
...
export OUT_DIR=$TXT_LIST/private/acl
export FILE_NAME='ban_ignore_ip_list'
export INGNORE_IP_FILE_NAME="$FILE_NAME"
export INGNORE_IP="$FILE_NAME"
export INGNORE_IP_TXT=$OUT_DIR/$FILE_NAME.txt
export INGNORE_IP_LOCAL_TXT=$OUT_DIR/$FILE_NAME-local-address.txt
export INGNORE_IP_CSV=$OUT_DIR/$FILE_NAME.csv
export INGNORE_IP_GREP=$OUT_DIR/$FILE_NAME.grep
export INGNORE_IP_UNI=$OUT_DIR/$FILE_NAME-uni.txt
export INGNORE_IP_TMOBILE=$OUT_DIR/$FILE_NAME-t-mobile.txt
export INGNORE_IP_SUBNET=$OUT_DIR/$FILE_NAME-subnet.txt
export INGNORE_IP_SUBNET_GREP=$OUT_DIR/$FILE_NAME-subnet.grep

INGORE_IP_ADRESSES_GREP=$( bash $PROG/grepify.sh $( printf '%s\n' "${DNS_IGNORE_IPs[@]}" | ip-sort | ip-subnet-ignore ) )
INGORE_IP_ADRESSES_SUBNET_GREP=$( bash $PROG/grepify.sh $( printf '%s\n' "${subnet_ignore_list[@]}" | ip-sort-subnet | ip-subnet-ignore ) )
INGORE_IP_ADRESSES_CSV=$( bash $PROG/csvify.sh $( printf '%s\n' "${DNS_IGNORE_IPs[@]}" | ip-sort-subnet | ip-subnet-ignore ) )


if [ "$IS_SOURCED" == 'false' ]; then
	echo "Bash non source writing files"
	new_linify "${DNS_IGNORE_IPs[@]}" | ip-sort-subnet | ip-subnet-ignore | sudo tee $INGNORE_IP_TXT
	new_linify "${UNI_IGNORE_IP[@]}" | ip-sort-subnet | ip-subnet-ignore | sudo tee $INGNORE_IP_UNI


	new_linify "${subnet_ignore_list[@]}" | ip-sort-subnet | ip-subnet-ignore | sudo tee $INGNORE_IP_SUBNET
	new_linify "${T_MobileIPs[@]}" | ip-sort-subnet |  ip-subnet-ignore | sudo tee $INGNORE_IP_TMOBILE
	new_linify "${local_ignore_ips[@]}" | ip-sort-subnet | ip-subnet-ignore | sudo tee $INGNORE_IP_LOCAL_TXT

	new_linify "$INGORE_IP_ADRESSES_SUBNET_GREP" | ip-sort-subnet | ip-subnet-ignore | sudo tee $INGNORE_IP_SUBNET_GREP
	new_linify "$INGORE_IP_ADRESSES_GREP" | ip-sort-subnet | ip-subnet-ignore | sudo tee $INGNORE_IP_GREP
	new_linify "$INGORE_IP_ADRESSES_CSV" | ip-sort-subnet | ip-subnet-ignore | sudo tee $INGNORE_IP_CSV
else
