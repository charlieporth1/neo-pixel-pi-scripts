...
sudo resolvconf -u
if ! [[ -L $RESCONF_FILE ]]; then
	echo NOT LINK
	echo "Relinking"
	sudo rm $RESCONF_FILE
	sudo ln -s /run/resolvconf/resolv.conf $RESCONF_FILE
fi
#if ! [[ -f $DNS_FILE ]]; then
#	sudo rm -rf $DNS_FILE
#	sudo touch $DNS_FILE
...
domain_to_replace="""us-central1-a.c.galvanic-pulsar-284521.internal
                      ...
                      google.internal
"""
domain_to_replace_regex="(us-central1-a.c.galvanic-pulsar-284521.internal|c.galvanic-pulsar-284521.internal|google.internal)"
	...
	for file in ${files[@]}
	do
		perl -0777 -i -pe "s/$domain_to_replace/$domains/g" $file
		perl -0777 -i -pe "s/$domain_to_replace_flat/$domains/g" $file
		perl -0777 -i -pe "s/$domain_to_replace_regex/$domains/g" $file
	done
fi
...
if [[ `ip_exists $DNS_SERVER` == 'true' ]]; then
	...
	[[ -z `grep -o "$DNS_SERVER" $DNS_FILE` ]] && echo "nameserver $DNS_SERVER" | sudo tee -a $DNS_FILfi
