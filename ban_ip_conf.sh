...
wait_time=5
IP_SET_DEFAULT_OPT="--match set --wait $wait_time"
...
#	IP_SET_DEFAULT_OPT=a
	...
	if ! printf "$IPSET_LIST" '%s\n' | grep -o "$IPSET_BK_NAME" > /dev/null; then
		echo "Table does not exist creating table $IPSET_BK_NAME"
	        ...
	        sudo iptables -I OUTPUT $IP_SET_DEFAULT_OPT --match-set $IPSET_BK_NAME src -j ACCEPT --table $ipset_table-is-table
	        sudo iptables -I INPUT $IP_SET_DEFAULT_OPT --match-set $IPSET_BK_NAME src -j ACCEPT --table $ipset_table-is-table
	        sudo iptables -I FORWARD $IP_SET_DEFAULT_OPT --match-set $IPSET_BK_NAME src -j ACCEPT --table $ipset_table-is-table
	else
...
#	IP_SET_DEFAULT_OPT=b
	ipset_table=b
        echo $IPSET_BK_NAME $IPSET_FILE
	if ! printf "$IPSET_LIST" '%s\n' | grep -o "$IPSET_BK_NAME" > /dev/null; then
		echo "Table does not exist creating table $IPSET_BK_NAME"
		IPSET_HASH_SIZE=$(( $IP_SUBNET_COUNT * 5 ))
	        sudo ipset create $IPSET_BK_NAME hash:net hashsize $IPSET_HASH_SIZE
	        sudo iptables -I INPUT $IP_SET_DEFAULT_OPT --match-set $IPSET_BK_NAME src -j DROP --table $ipset_table-is-table
	        sudo iptables -I FORWARD $IP_SET_DEFAULT_OPT --match-set $IPSET_BK_NAME src -j DROP --table $ipset_table-is-table
	else
		echo "Table exist using table $IPSET_BK_NAME"
	fi
...
export -f run_ip-set-block-file


function run_ip-set-block-file_abs_subnet() {
	local full_url_file_path="$1"
	load_file $full_url_file_path

	for ip in "${ban_ips[@]}"
	do
        	ipset add $IPSET_BK_NAME $ip/18 > /dev/null

	done
	save_ip-set $IPSET_BK_NAME $IPSET_FILE_FULL
}
export -f run_ip-set-block-file_abs_subnet

function get_list_of_ignore_lists() {
	curl 'http://100.71.239.28/acl/list_acl_list.php'
}
export -f get_list_of_ignore_lists
